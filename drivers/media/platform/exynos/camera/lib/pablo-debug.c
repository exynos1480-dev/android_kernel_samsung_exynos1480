...
static int debug_param[IS_DEBUG_PARAM_MAX];
module_param_named(debug_clk, debug_param[IS_DEBUG_PARAM_CLK], int, S_IRUGO | S_IWUSR);
module_param_named(debug_stream, debug_param[IS_DEBUG_PARAM_STREAM], int, S_IRUGO | S_IWUSR);
module_param_named(debug_video, debug_param[IS_DEBUG_PARAM_VIDEO], int, S_IRUGO | S_IWUSR);
module_param_named(debug_hw, debug_param[IS_DEBUG_PARAM_HW], int, S_IRUGO | S_IWUSR);
module_param_named(debug_device, debug_param[IS_DEBUG_PARAM_DEVICE], int, S_IRUGO | S_IWUSR);
module_param_named(debug_irq, debug_param[IS_DEBUG_PARAM_IRQ], int, S_IRUGO | S_IWUSR);
module_param_named(debug_csi, debug_param[IS_DEBUG_PARAM_CSI], int, S_IRUGO | S_IWUSR);
module_param_named(debug_time_launch, debug_param[IS_DEBUG_PARAM_TIME_LAUNCH], int, S_IRUGO | S_IWUSR);
module_param_named(debug_time_queue, debug_param[IS_DEBUG_PARAM_TIME_QUEUE], int, S_IRUGO | S_IWUSR);
module_param_named(debug_time_shot, debug_param[IS_DEBUG_PARAM_TIME_SHOT], int, S_IRUGO | S_IWUSR);
module_param_named(debug_s2d, debug_param[IS_DEBUG_PARAM_S2D], int, S_IRUGO | S_IWUSR);
module_param_named(debug_dvfs, debug_param[IS_DEBUG_PARAM_DVFS], int, S_IRUGO | S_IWUSR);
module_param_named(debug_mem, debug_param[IS_DEBUG_PARAM_MEM], int, S_IRUGO | S_IWUSR);
module_param_named(debug_lvn, debug_param[IS_DEBUG_PARAM_LVN], int, S_IRUGO | S_IWUSR);
module_param_named(debug_llc, debug_param[IS_DEBUG_PARAM_LLC], int, S_IRUGO | S_IWUSR);
module_param_named(debug_yuvp, debug_param[IS_DEBUG_PARAM_YUVP], int, S_IRUGO | S_IWUSR);
module_param_named(debug_shrp, debug_param[IS_DEBUG_PARAM_SHRP], int, S_IRUGO | S_IWUSR);
module_param_named(debug_iq, debug_param[IS_DEBUG_PARAM_IQ], int, S_IRUGO | S_IWUSR);
module_param_named(debug_iq_cr_dump, debug_param[IS_DEBUG_PARAM_IQ_CR_DUMP], int, S_IRUGO | S_IWUSR);
module_param_named(debug_phy_tune, debug_param[IS_DEBUG_PARAM_PHY_TUNE], int, 0644);
module_param_named(debug_crc_seed, debug_param[IS_DEBUG_PARAM_CRC_SEED], int, 0644);
module_param_named(debug_sensor, debug_param[IS_DEBUG_PARAM_SENSOR], int, 0644);
module_param_named(debug_assert_crash, debug_param[IS_DEBUG_PARAM_ASSERT_CRASH], int, 0644);
module_param_named(debug_ixc, debug_param[IS_DEBUG_PARAM_IXC], int, 0644);
...
static int draw_digit_set(const char *val, const struct kernel_param *kp)
{
	...
	if (argc == 1) {
		ret = kstrtouint(argv[0], 0, &debug_param[IS_DEBUG_PARAM_DRAW_DIGIT]);
	} else if (argc > 4) {
...
module_param_cb(draw_digit, &draw_digit_ops, &debug_param[IS_DEBUG_PARAM_DRAW_DIGIT], 0644);
		...
		u32 height, ulong addr)
{
	...
	} else if (debug_param[IS_DEBUG_PARAM_DRAW_DIGIT] > 1) {
...
void is_dbg_draw_digit(struct is_debug_dma_info *dinfo, u64 digit)
{
	...
	case V4L2_PIX_FMT_GREY:
		...
		break;
	default:
		if (debug_param[IS_DEBUG_PARAM_DRAW_DIGIT] > 1)
...
int is_debug_memlog_alloc_dump(phys_addr_t paddr, size_t size, const char *name)
{
	...
	is_debug.memlog.dump[idx].obj = CALL_PML_OPS(is_debug.memlog, alloc_dump,
			is_debug.memlog.desc, size, paddr, true, NULL, dump_name);
	if (!is_debug.memlog.dump[idx].obj) {
...
void is_debug_memlog_dump_cr_all(int log_level)
{
	...
	for (i = 0; i < nums; i++) {
		if (is_debug.memlog.dump[i].obj) {
			CALL_PML_OPS(is_debug.memlog, do_dump, is_debug.memlog.dump[i].obj, log_level);
			info("[MEMLOG][Lv:%d][I:%d] %s CR dumped(addr:%pad, size:0x%zx)",
					log_level, i,
					is_debug.memlog.dump[i].name,
					&is_debug.memlog.dump[i].paddr,
					is_debug.memlog.dump[i].size);
		}
	}
}
...
static void _is_debug_handle_overflow_dma(u32 instance, u32 fcount)
{
	...
	if (debug_param[IS_DEBUG_PARAM_PHY_TUNE])
		return;
...
#if IS_ENABLED(CONFIG_EXYNOS_BCM_DBG)
	if (en) {
		exynos_bcm_dbg_change_event_format(4);
		CALL_PBCM_OPS(is_debug, start);
	} else {
		CALL_PBCM_OPS(is_debug, stop, bcm_owner);
		exynos_bcm_dbg_restore_event_format();
	}
#endif
}
...
KUNIT_EXPORT_SYMBOL(pablo_debug_get_event);
#endif
