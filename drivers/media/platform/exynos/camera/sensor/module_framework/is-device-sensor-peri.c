...
int is_sensor_flush_prev_mode_change(struct is_cis *cis)
{
	...
	dbg_sensor(1, "flush_work mode_setting_work\n");
	flush_work(&sensor_peri->cis.mode_setting_work);
...
#ifndef USE_EARLY_OIS_INIT
	if (sensor_peri->mcu && sensor_peri->mcu->ois)
		schedule_work(&sensor_peri->mcu->ois->set_init_work);
#endif
#endif
	return ret;
}
...
void is_sensor_setting_mode_change(struct is_device_sensor_peri *sensor_peri)
{
	...
	is_sensor_peri_update_seamless_mode(device);
...
void is_sensor_flash_set_pre_config_work(struct work_struct *data)
{
	int ret = 0;
	struct is_flash *flash;
	struct is_flash_data *flash_data;
	...
	flash_data = container_of(data, struct is_flash_data, flash_set_pre_config_work);
	flash = container_of(flash_data, struct is_flash, flash_data);

	sensor_peri = flash->sensor_peri;

	if (!sensor_peri->subdev_flash) {
		err("err!!! sensor_peri->subdev_flash is null");
		return;
	}

	subdev_flash = sensor_peri->subdev_flash;

	ctrl.id = V4L2_CID_FLASH_SET_TORCH_PRE_CONFIG;
	ctrl.value = 0;
...
void is_sensor_actuator_active_off_work(struct work_struct *data)
{
	...
	if (atomic_read(&core->resourcemgr.global_param.sensor_cnt) > 1
			|| sensor_peri->cis.cis_data->video_mode) {
...
void is_sensor_ois_set_init_work(struct work_struct *data)
{
	...
	ois = container_of(data, struct is_ois, set_init_work);
	...
	ret = CALL_OISOPS(sensor_peri->mcu->ois, ois_set_power_mode, sensor_peri->subdev_mcu, OIS_USE_UW_NONE);
	if (ret < 0)
...
#if defined(CONFIG_CAMERA_USE_EXTERNAL_MCU) || defined(CONFIG_CAMERA_USE_AOIS)
	CALL_OISOPS(sensor_peri->mcu->ois, ois_fw_update, is_get_is_core());
#endif
...
void is_sensor_ois_set_deinit_work(struct work_struct *data)
{
	...
	ois = container_of(data, struct is_ois, set_deinit_work);
...
int is_sensor_peri_notify_vsync(struct v4l2_subdev *subdev, void *arg)
{
	...
	CALL_CISOPS(cis, cis_update_seamless_state, sensor_peri->subdev_cis);
	...
	if (laser_af && test_bit(IS_SENSOR_LASER_AF_AVAILABLE, &sensor_peri->peri_state)
		&& laser_af->active) {
		/* run laf thread */
		queue_work(laser_af->workqueue, &laser_af->get_distance_work);
	}
...
int is_sensor_peri_notify_vblank(struct v4l2_subdev *subdev, void *arg)
{
	...
	vblank_count = *(u32 *)arg;

	cis = (struct is_cis *)v4l2_get_subdevdata(sensor_peri->subdev_cis);
	cis->cis_data->sen_vblank_count = vblank_count;
...
void is_sensor_peri_m2m_actuator(struct work_struct *data)
{
	...
	cur_frame_cnt = device->ischain->group[GROUP_SLOT_3AA]->fcount;
...
void is_sensor_cis_mode_setting_work(struct work_struct *data)
{
	...
	if (sensor_peri->subdev_cis) {
		...
		ret = CALL_CISOPS(cis, cis_init_state, cis->subdev, cis->cis_data->sens_config_index_cur);
		if (ret < 0) {
			err("err!!! init_state ret(%d)", ret);
			return;
		}
...
void is_sensor_peri_init_work(struct is_device_sensor_peri *sensor_peri)
{
	...
	if (sensor_peri->flash) {
		...
		if (sensor_peri->flash->use_pre_config_work)
			INIT_WORK(&sensor_peri->flash->flash_data.flash_set_pre_config_work,
				is_sensor_flash_set_pre_config_work);
	}
	...
	if (sensor_peri->mcu && sensor_peri->mcu->ois) {
		INIT_WORK(&sensor_peri->mcu->ois->set_init_work, is_sensor_ois_set_init_work);
#if defined(USE_TELE_OIS_AF_COMMON_INTERFACE) || defined(USE_TELE2_OIS_AF_COMMON_INTERFACE)
		INIT_WORK(&sensor_peri->mcu->ois->set_deinit_work, is_sensor_ois_set_deinit_work);
#endif
	}
...
void is_sensor_peri_probe(struct is_device_sensor_peri *sensor_peri)
{
	...
	mutex_init(&sensor_peri->cis.stream_off_lock);
}
...
#if defined(CONFIG_CAMERA_USE_INTERNAL_MCU) && defined(CAMERA_2ND_OIS)
int is_sensor_peri_check_mcu_disable_condition(struct is_device_sensor *device, bool on)
{
	...
	if (!device_mcu) {
		err("device_mcu is NULL");
		return ret;
	}

	if ((device->ischain->setfile & IS_SETFILE_MASK) == ISS_SUB_SCENARIO_SUPER_STEADY_FHD_30FPS_VIDEO
		|| (sensor_peri->check_auto_framing)) {
		if ((device_mcu->mcu && device_mcu->mcu->off_during_uwonly_mode)
			|| (sensor_peri->check_auto_framing && !sensor_peri->mcu && !sensor_peri->subdev_mcu)) {
			ret = OIS_USE_UW_ONLY;
		} else {
			ret = OIS_USE_UW_WIDE;
		}
					...
					bool on)
{
	...
	struct is_core *core;
	int sensor_cnt;
#if defined(CONFIG_OIS_USE) && defined(CAMERA_2ND_OIS)
	struct is_device_sensor *device_mcu = NULL;
	...
	dbg_sensor(1, "[%s] on = %d, scene_mode = %d, opening_hint = %d",
		__func__, on, cis->cis_data->is_data.scene_mode, core->vendor.opening_hint);

	if (cis->cis_data->is_data.scene_mode == AA_SCENE_MODE_FAST_AE
		&& core->vendor.opening_hint == IS_OPENING_HINT_FASTEN_AE) {
	...
	if (cis->cis_data->is_data.scene_mode == AA_SCENE_MODE_FAST_AE
		&& core->vendor.opening_hint == IS_OPENING_HINT_FASTEN_AE_VIDEO) {
	...
	if (on) {
		...
		if (check_mcu_disable) {
			...
			if ((device_mcu && device_mcu->mcu && device_mcu->subdev_mcu) || (sensor_peri->mcu && sensor_peri->subdev_mcu)) {
				if (!sensor_peri->mcu) {
					if (check_mcu_disable == OIS_USE_UW_ONLY) {
						ret = CALL_OISOPS(device_mcu->mcu->ois, ois_set_power_mode, device_mcu->subdev_mcu, OIS_USE_UW_ONLY);
					} else {
						ret = CALL_OISOPS(device_mcu->mcu->ois, ois_set_power_mode, device_mcu->subdev_mcu, OIS_USE_UW_WIDE);
					}

					current_mcu = device_mcu->mcu;
					subdev_mcu = device_mcu->subdev_mcu;
					info("[%s] Use default mcu.\n", __func__);
				...
				if (current_mcu && subdev_mcu) {
					...
					info("[%s] ois disable in case of supersteady/auto framing scenario. pos = %d\n", __func__, sensor_peri->module->position);
				} else {
		...
		if (core->vendor.opening_hint == IS_OPENING_HINT_TO_REMOSAIC) {
			info("[%s] opening hint to remosaic, skip sub device\n", __func__);
			skip_sub_device = true;
		...
		if (!skip_sub_device_mcu && !check_mcu_disable) {
			...
			if (sensor_peri->mcu && sensor_peri->mcu->ois)
				flush_work(&sensor_peri->mcu->ois->set_init_work);
...
#endif
		ret = CALL_CISOPS(cis, cis_set_flip_mode, subdev_cis);
		if (ret < 0)
			err("err!!! cis set flip mode ret(%d)", ret);

		ret = CALL_CISOPS(cis, cis_update_seamless_state, subdev_cis);
		if (ret < 0)
		...
		is_sec_update_hw_param_af_info(device, 0, HW_PARAM_STREAM_ON);
#endif

	} else {
		if (core->vendor.closing_hint == IS_CLOSING_HINT_FROM_TETRA) {
			info("[%s] closing hint from tetra, skip sub device\n", __func__);
			skip_sub_device = true;
			core->vendor.closing_hint = IS_CLOSING_HINT_NONE;
		}

#if IS_ENABLED(CONFIG_CAMERA_HW_BIG_DATA)
		is_sec_update_hw_param_af_info(device, 0, HW_PARAM_STREAM_OFF);
#endif
		...
		if (!skip_sub_device_mcu && !check_mcu_disable) {
			if (sensor_peri->mcu && sensor_peri->mcu->ois)
				schedule_work(&sensor_peri->mcu->ois->set_deinit_work);
		}
#endif
		...
		if (!skip_sub_device_mcu && !check_mcu_disable) {
			...
			if (sensor_peri->mcu && sensor_peri->mcu->ois)
				flush_work(&sensor_peri->mcu->ois->set_deinit_work);
...
int is_sensor_peri_update_seamless_mode(struct is_device_sensor *device)
{
	...
	u64 cur_vsync_ts, start_ts, end_ts;
	...
	min_frame_us_time_backup = cis_data->min_frame_us_time;

	if (cis_data->sen_vsync_count)
		cur_vsync_ts = device->timestamp[cis_data->sen_vsync_count % IS_TIMESTAMP_HASH_KEY];
	else
		cur_vsync_ts = 0;
	...
	if (cis_data->min_frame_us_time < min_frame_us_time_backup)
		cis_data->min_frame_us_time = min_frame_us_time_backup;

	if (cis_data->seamless_mode_changed) {
		/* update_seamless_state should be updated after group param hold is released*/
		if (cis_data->group_param_hold == 0x0) {
			cis_data->update_seamless_state = true;
			cis_data->seamless_mode_changed = false;
		}

		cis_data->seamless_update_vsync_cnt = cis_data->sen_vsync_count;

		info("%s [%d][%s] SS_DBG GPH[%d] changed[%d] update[%d] AE delay[%lldus] seamless delay[%lldus]\n",
			__func__, cis_data->sen_vsync_count, cis->sensor_info->name,
			cis_data->group_param_hold, cis_data->seamless_mode_changed,
			cis_data->update_seamless_state, (start_ts - cur_vsync_ts) / 1000, (end_ts - start_ts) / 1000);
	}
...
void is_sensor_peri_compensate_duration_for_frame_sync(struct is_device_sensor *device, u32 vsync_count)
{
...
#ifdef USE_CAMERA_MCD_SW_DUAL_SYNC
	frame_interval = vsync_count - sensor_peri->cis.cis_data->dual_sync_count;
	sensor_peri->cis.cis_data->dual_sync_count = vsync_count;
#endif
	...
	/* SW dual sync WA */
	if (frame_interval == 2) {
		dbg_sensor(2, "[%s][F:%d] frame_interval(%d)", __func__, vsync_count, frame_interval);
		compensate_duration = (sensor_uctrl->dynamicFrameDuration + compensate_duration) / 2;
	}
#endif

	dbg_sensor(2, "[%s][F:%d] Duration is adjusted(%lld) for frame sync cur_duration_m(%lld)",
		__func__, vsync_count, compensate_duration, cur_duration_m);
