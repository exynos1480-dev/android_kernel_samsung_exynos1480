...
static const u32 mipi_testmode_951mbps[] = {
	0x0808, 0x02, 0x01,
	0x080A, 0x00, 0x01,
	0x080B, 0x7F, 0x01,
	0x080C, 0x00, 0x01,
...
static const u32 mipi_default_mode[] = {
	0x0808, 0x00, 0x01,
	0x080A, 0x00, 0x01,
	0x080B, 0x00, 0x01,
	0x080C, 0x00, 0x01,
...
int sensor_imx616_cis_stream_on(struct v4l2_subdev *subdev)
{
	...
	if ((cis->cis_data->sens_config_index_cur == SENSOR_IMX616_MODE_2X2BIN_3264x2448_30FPS)
		&& (cis->mipi_clock_index_cur == CAM_IMX616_SET_A_2x2bin_4_3_30fps_951p8_MHZ)) {
		ret = sensor_cis_set_registers(subdev, mipi_testmode_951mbps, ARRAY_SIZE(mipi_testmode_951mbps));
	} else {
		ret = sensor_cis_set_registers(subdev, mipi_default_mode, ARRAY_SIZE(mipi_default_mode));
	}
...
#endif
	cis->ixc_ops->read8(cis->client, 0x0808, &ctrl_mode);
	/* Sensor stream on */
	info("%s (ctrl_mode:0x%x)\n", __func__, ctrl_mode);
