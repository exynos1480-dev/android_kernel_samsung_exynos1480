...
int sensor_cis_init_state(struct v4l2_subdev *subdev, int mode)
{
	...
	info("%s [%d][%d][%s] SS_DBG LN2[%d] LN4[%d] 12bit[%d] AEB[%d] RM[%d,%d]\n",
		...
		(init_seamless_mode.mode & SENSOR_MODE_LN2),
		(init_seamless_mode.mode & SENSOR_MODE_LN4),
		(init_seamless_mode.mode & SENSOR_MODE_REAL_12BIT),
		(init_seamless_mode.mode & SENSOR_MODE_AEB),
		(init_seamless_mode.mode & SENSOR_MODE_CROPPED_RMS),
		init_seamless_mode.remosaic_crop_zoom_ratio);
...
int sensor_cis_update_seamless_state(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct is_cis *cis = sensor_cis_get_cis(subdev);
	cis_shared_data *cis_data;
	struct is_sensor_seamless_ctl new_seamless_mode;
	...
	if (cis_data->pre_remosaic_zoom_ratio != cis_data->cur_remosaic_zoom_ratio) {
		cis_data->remosaic_crop_update_vsync_cnt = cis_data->sen_vsync_count;
		info("%s [%d][%d][%s] SS_DBG rmc_update_vsync_cnt(%d)\n",
			__func__, cis->id, cis_data->sen_vsync_count, cis->sensor_info->name,
	...
	memset(&new_seamless_mode, 0, sizeof(struct is_sensor_seamless_ctl));

	if (cis_data->pre_lownoise_mode == IS_CIS_LN2)
		new_seamless_mode.mode |= SENSOR_MODE_LN2;
	else if (cis_data->pre_lownoise_mode == IS_CIS_LN4)
		new_seamless_mode.mode |= SENSOR_MODE_LN4;

	if (cis_data->pre_12bit_mode == SENSOR_12BIT_STATE_REAL_12BIT)
		new_seamless_mode.mode |= SENSOR_MODE_REAL_12BIT;

	if (cis_data->pre_hdr_mode == SENSOR_HDR_MODE_2AEB_2VC)
		new_seamless_mode.mode |= SENSOR_MODE_AEB;

	if (cis_data->pre_remosaic_zoom_ratio > 0) {
		new_seamless_mode.mode |= SENSOR_MODE_CROPPED_RMS;
		new_seamless_mode.remosaic_crop_zoom_ratio = cis_data->pre_remosaic_zoom_ratio;
	}

	memcpy(&cis_data->seamless_mode_history[cis_data->sen_vsync_count % NUM_FRAMES],
		&new_seamless_mode, sizeof(struct is_sensor_seamless_ctl));
...
EXPORT_SYMBOL_GPL(sensor_cis_update_seamless_state);
...
int sensor_cis_set_group_param_hold(struct v4l2_subdev *subdev, bool hold)
{
	...
	if (cis_data->seamless_mode_changed && hold == false) {
		cis_data->update_seamless_state = true;
		cis_data->seamless_mode_changed = false;
		cis_data->skip_control_vsync_count = cis_data->sen_vsync_count;
	}
...
int sensor_cis_set_flip_mode(struct v4l2_subdev *subdev)
{
	...
	info("[%s] sensor flip mode (%d)", __func__, flip_mode);
	...
	enum is_rta_sensor_mode sensor_mode, u32 value)
{
	...
	info("[%s][%d][%s] cur_mode(%d), LN2[%d] LN4[%d] 12bit[%d] AEB[%d] RM[%d,%d]\n",
		__func__, cis_data->sen_vsync_count, cis->sensor_info->name,
		cis_data->sens_config_index_cur,
		(cis_data->seamless_ctl_before_stream.mode & SENSOR_MODE_LN2),
		(cis_data->seamless_ctl_before_stream.mode & SENSOR_MODE_LN4),
		(cis_data->seamless_ctl_before_stream.mode & SENSOR_MODE_REAL_12BIT),
		(cis_data->seamless_ctl_before_stream.mode & SENSOR_MODE_AEB),
		(cis_data->seamless_ctl_before_stream.mode & SENSOR_MODE_CROPPED_RMS),
