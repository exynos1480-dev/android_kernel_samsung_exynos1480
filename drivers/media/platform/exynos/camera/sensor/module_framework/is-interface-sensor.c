		...
		u32 *exposure_count)
{
	...
	if (sensor->cfg->input[CSI_VIRTUAL_CH_0].hwformat == HW_FORMAT_RAW12) {
...
u32 set_sensor_12bit_state(struct is_sensor_interface *itf, enum is_sensor_12bit_state state)
{
	...
	if (cis_data->cur_12bit_mode != cis_data->pre_12bit_mode)
		info("[%s][%d][%d][%s] AE_CTL pre(%d)->cur(%d)\n", __func__, sensor_peri->cis.id,
			cis_data->sen_vsync_count, sensor_peri->module->sensor_name,
		...
		camera2_udm_t *udm)
{
	...
	dbg_sensor(1, "[%s][%d][%d][%s] expo(%lld), duration(%lld), sensitivity(%d), rollingShutterSkew(%lld)\n",
	...
	struct is_sensor_interface *itf, enum itf_vc_buf_data_type request_data_type)
{
	...
	case VC_BUF_DATA_TYPE_SENSOR_STAT1:
		...
		for (wdma_vc = DMA_VIRTUAL_CH_0; wdma_vc < DMA_VIRTUAL_CH_MAX; wdma_vc++) {
			if (sensor->cfg->output[0][wdma_vc].type == VC_EMBEDDED2) {
	...
	if (!subdev || !test_bit(IS_SUBDEV_START, &subdev->state)) {
		for (wdma_vc = DMA_VIRTUAL_CH_0; wdma_vc < DMA_VIRTUAL_CH_MAX; wdma_vc++) {
			if (sensor->cfg->output[0][wdma_vc].type == vc_type)
				break;
		}
		...
		return GET_SUBDEV_I_FRAMEMGR(&csi->i_subdev[0][wdma_vc]);
	}

	return GET_SUBDEV_I_FRAMEMGR(subdev);
}

static int update_pd_vc_buf_masking(
	struct v4l2_subdev *v4l2_subdev, struct is_frame *frame, bool use)
{
	int ret;
		...
		u64 *buf_addr)
{
	...
	*buf_index = 0;
	...
	if (frame) {
		struct is_priv_buf *pb = frame->pb_output;
	...
	if (request_data_type == VC_BUF_DATA_TYPE_SENSOR_STAT1)
		update_pd_vc_buf_masking(sensor->subdev_csi, frame, 1);
		...
		u32 frame_count)
{
	...
	if (request_data_type == VC_BUF_DATA_TYPE_SENSOR_STAT1)
		update_pd_vc_buf_masking(sensor->subdev_csi, frame, 0);
		...
		u32 index)
{
	...
	if (index >= framemgr->num_frames) {
		mserr("index(%d of %d) is out-of-range", sensor, framemgr, index,
			framemgr->num_frames);
		return -ENOENT;
	}
	...
	dbg_sensor(
		1, "[%s]: [%d][%s] index: %d\n", __func__, frame->instance, framemgr->name, index);

	return 0;
}

u32 get_link_vc_by_type(struct is_device_sensor *sensor, u32 type)
{
	u32 idx_wdma, wdma_vc;

	for (idx_wdma = 0; idx_wdma < sensor->cfg->dma_num; idx_wdma++) {
		for (wdma_vc = DMA_VIRTUAL_CH_0; wdma_vc < DMA_VIRTUAL_CH_MAX; wdma_vc++) {
			if (sensor->cfg->output[CSI_OTF_OUT_SINGLE][wdma_vc].type == type)
				return sensor->cfg->link_vc[idx_wdma][wdma_vc];
		}
	}

	return CSI_VIRTUAL_CH_MAX;
}
		...
		struct vc_buf_info_t *buf_info)
{
	...
	case VC_BUF_DATA_TYPE_SENSOR_STAT1:
		for (ch = DMA_VIRTUAL_CH_1; ch < DMA_VIRTUAL_CH_MAX; ch++) {
			if (sensor->cfg->output[0][ch].type == VC_TAILPDAF)
				break;
		}
		break;
	case VC_BUF_DATA_TYPE_SENSOR_STAT2:
		for (ch = DMA_VIRTUAL_CH_1; ch < DMA_VIRTUAL_CH_MAX; ch++) {
			if (sensor->cfg->output[0][ch].type == VC_EMBEDDED)
				break;
		}
		break;
	case VC_BUF_DATA_TYPE_GENERAL_STAT1:
		for (ch = DMA_VIRTUAL_CH_1; ch < DMA_VIRTUAL_CH_MAX; ch++) {
			if (sensor->cfg->output[0][ch].type == VC_PRIVATE)
				break;
		}
		break;
	case VC_BUF_DATA_TYPE_GENERAL_STAT2:
		for (ch = DMA_VIRTUAL_CH_1; ch < DMA_VIRTUAL_CH_MAX; ch++) {
			if (sensor->cfg->output[0][ch].type == VC_MIPISTAT)
				break;
		}
		break;
	case VC_BUF_DATA_TYPE_SENSOR_STAT3:
		for (ch = DMA_VIRTUAL_CH_1; ch < DMA_VIRTUAL_CH_MAX; ch++) {
			if (sensor->cfg->output[0][ch].type == VC_VPDAF)
				break;
		}
		break;
	default:
	...
	stat_vc = sensor->cfg->hpd_vc[CSI_OTF_OUT_SINGLE];
	...
	buf_info->width = sensor->cfg->input[stat_vc].width;
...
int set_hdr_mode(struct is_sensor_interface *itf, u32 mode)
{
	...
	if (!cis_data->stream_on && mode != SENSOR_HDR_MODE_SINGLE) {
		info("[%s][%d][%d][%s] skip & reset AEB control during stream off (pre: %d, cur:%d, mode:%d)\n",
			__func__, sensor_peri->cis.id,
			cis_data->sen_vsync_count, sensor_peri->module->sensor_name,
			cis_data->pre_hdr_mode, cis_data->cur_hdr_mode, mode);
...
int get_sensor_applied_seamless_mode(struct is_sensor_interface *itf,
				struct is_sensor_seamless_ctl *applied_seamless_mode)
{
	struct is_device_sensor_peri *sensor_peri;
	cis_shared_data *cis_data;
	...
	if (sensor_peri->cis.ctrl_delay == N_PLUS_ONE_FRAME) {
		ref_history_index = (cis_data->sen_vsync_count % NUM_FRAMES);
		ctrl_delay = 1;
	} else {
		ref_history_index = ((NUM_FRAMES + cis_data->sen_vsync_count - 1) % NUM_FRAMES);
		ctrl_delay = 2;
	}

	memcpy(applied_seamless_mode,
		&cis_data->seamless_mode_history[ref_history_index % NUM_FRAMES],
		sizeof(struct is_sensor_seamless_ctl));

	if (!cis_data->stream_on)
		applied_seamless_mode->mode &= ~SENSOR_MODE_AEB;

	if (cis_data->seamless_update_vsync_cnt + ctrl_delay == cis_data->sen_vsync_count)
		info("%s [%d][%d][%s] SS_DBG LN2[%d] LN4[%d] 12bit[%d] AEB[%d] RM[%d,%d]\n",
			__func__, sensor_peri->cis.id,
			cis_data->sen_vsync_count, sensor_peri->module->sensor_name,
			(applied_seamless_mode->mode & SENSOR_MODE_LN2),
			(applied_seamless_mode->mode & SENSOR_MODE_LN4),
			(applied_seamless_mode->mode & SENSOR_MODE_REAL_12BIT),
			(applied_seamless_mode->mode & SENSOR_MODE_AEB),
			(applied_seamless_mode->mode & SENSOR_MODE_CROPPED_RMS),
			applied_seamless_mode->remosaic_crop_zoom_ratio);
	else
		dbg_sensor(1, "%s [%d][%d][%s] SS_DBG LN2[%d] LN4[%d] 12bit[%d] AEB[%d] RM[%d,%d]\n",
			__func__, sensor_peri->cis.id,
			cis_data->sen_vsync_count, sensor_peri->module->sensor_name,
			(applied_seamless_mode->mode & SENSOR_MODE_LN2),
			(applied_seamless_mode->mode & SENSOR_MODE_LN4),
