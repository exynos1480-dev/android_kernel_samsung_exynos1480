...
static void is_sensor_instanton(struct work_struct *data)
{
	...
	if (instant_cnt) {
		...
		timetowait =
			wait_event_timeout(group->instant_wait, (!group->instant_cnt), timeout);
		if (!timetowait) {
		...
		minfo("[SS%d:D] instant off(fcount : %d, time : %ldms)\n", device,
			device->device_id, group->instant_cnt, timetoelapse);
...
static int is_sensor_probe(struct platform_device *pdev)
{
	...
	for (idx_wdma = 0; idx_wdma < CSIS_MAX_NUM_DMA_ATTACH; idx_wdma++) {
		for (vc = DMA_VIRTUAL_CH_0; vc < DMA_VIRTUAL_CH_MAX; vc++) {
			link_vc = vc + idx_wdma * DMA_VIRTUAL_CH_MAX;

#if !IS_ENABLED(CONFIG_SENSOR_GROUP_LVN)
			group->subdev[ENTRY_SSVC0 + link_vc] = &device->ssvc[idx_wdma][vc];
			list_add_tail(&device->ssvc[idx_wdma][vc].list, &group->subdev_list);
#endif

			is_subdev_probe(&device->ssvc[idx_wdma][vc], device_id,
					link_vc + ENTRY_SSVC0, vc_names[idx_wdma][vc],
...
static int is_sensor_back_start(void *device, struct is_queue *iq)
{
	...
	for (idx_wdma = 0; idx_wdma < csi->sensor_cfg->dma_num; idx_wdma++) {
		for (wdma_vc = DMA_VIRTUAL_CH_0; wdma_vc < DMA_VIRTUAL_CH_MAX; wdma_vc++) {
			struct pablo_internal_subdev *dma_subdev;

			dma_subdev = &csi->i_subdev[idx_wdma][wdma_vc];
			if (dma_subdev->size[0]) {
				ret = CALL_I_SUBDEV_OPS(dma_subdev, alloc, dma_subdev);
				if (ret) {
					mcerr("[CSI%d][VC%d][%s] failed to alloc(%d)", csi,
						csi->otf_info.csi_ch, wdma_vc, dma_subdev->name,
						ret);
					ret = -EINVAL;
...
p_err_internal_start:
	while (--idx_wdma >= 0) {
		while (--wdma_vc >= CSI_VIRTUAL_CH_0) {
			...
			dma_subdev = &csi->i_subdev[idx_wdma][wdma_vc];
			if (test_bit(PABLO_SUBDEV_ALLOC, &dma_subdev->state)) {
				if (icpu_dev) {
					framemgr = GET_SUBDEV_I_FRAMEMGR(dma_subdev);
					for (frame_idx = 0; frame_idx < framemgr->num_frames;
					     frame_idx++) {
				...
				if (CALL_I_SUBDEV_OPS(dma_subdev, free, dma_subdev))
					mcerr("[WDMA%d][VC%d][%s] failed to free", csi,
						csi->wdma[idx_wdma]->ch, wdma_vc, dma_subdev->name);
			}
		}
	...
	u32 nonblock)
{
	...
	device->group_sensor.instant_cnt = instant_cnt;
	subdev_csi = device->subdev_csi;
	subdev_module = device->subdev_module;
	if (!subdev_module) {
		merr("subdev_module is NULL", device);
